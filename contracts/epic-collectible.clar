;; Epic Collectible NFT Contract
;; Implements SIP-009 NFT standard
;; Supply: 5,000 | Mint Price: 0.05 STX

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

;; Define the NFT
(define-non-fungible-token epic-collectible uint)
...
;; Pricing: 0.05 STX = 50,000 microSTX
(define-constant MINT-PRICE u50000)
(define-constant MAX-SUPPLY u5000)

;; Data Variables
(define-data-var last-token-id uint u0)
(define-data-var base-uri (string-ascii 256) "https://mintmart.io/api/metadata/epic/")
...
(define-read-only (get-owner (token-id uint))
    (ok (nft-get-owner? epic-collectible token-id)))

(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin
        (asserts! (is-eq tx-sender sender) ERR-NOT-AUTHORIZED)
        (nft-transfer? epic-collectible token-id sender recipient)))
...
(define-public (mint)
    (let ((next-id (+ (var-get last-token-id) u1)))
        ...
        (try! (nft-mint? epic-collectible next-id tx-sender))
